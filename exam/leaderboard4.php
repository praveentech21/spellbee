...
<?php
	...
	$my_res=mysqli_query($conn, "SELECT sum(marks) as mymark from responses1 where sid='$ssid';"); 		
    $mrow=mysqli_fetch_array($my_res);
	$my_score=$mrow['mymark'];
	$table=$table."<h4><b>MY SCORE</b></h4><h1 style='color:#0047AB'><b>$my_score</b></h1>"; 

	
	$ques=mysqli_query($conn, "SELECT * from responses1 where sid='$ssid' order by timestamp LIMIT 35;"); 
	    ...
	    while($row=mysqli_fetch_array($ques))
	      {
			  $marks=$row['marks'];
			  $answer=$row['answer'];
			  if($qno <= 20){$word = mysqli_fetch_assoc(mysqli_query($conn,"select word from words where qid='{$row['qid']}'"))['word'];}
