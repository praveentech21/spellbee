...
if(isset($_GET['gid']))
{
	...
	$gm_res=mysqli_query($conn, "SELECT gid FROM games where gid=$gid;"); 
	if(mysqli_num_rows($gm_res) == 0) 
	{
      header("Location:dashboard.php");
    }
...
<html class="sidebar-light fixed sidebar-left-collapsed">
	...
	.ui-pnotify.green .ui-pnotify-container {
	 ...
	 var chk=0;
		...
		<section class="body">
	        ...
	        <?php include "sidebar.php"; ?>
				...
				<section role="main" class="content-body">
					...
					<div class="row">
						<div class="col-lg-8 mb-4">
        					...
        					$game_res=mysqli_query($conn, "SELECT gid, DATE_FORMAT(schedule, '%D %M [%a]') as dt, DATE_FORMAT(schedule, '%h:%i %p') as tm, status, cost, DATE_FORMAT(schedule, '%M %e, %Y %T') as gt FROM games where gid=$gid;"); 		
							$row=mysqli_fetch_array($game_res);
							
							$game_time=$row['gt'];
							echo "<h2 class='card-title mt-3' align='center'>GAME NO: $gid </h2>";

							echo "<h4 align='center' style='color:#0088cc;text-transform:uppercase;'><b>".$row[1]." - ".$row[2]."</b></h4>";
                            ?>
							...
							if($tid == 0)
							  {
   								    echo "<h5 align='center'>YOU HAVE NOT JOINED THIS GAME!</h5>";
  		        ...
  		        <h4 class="box-title" id="tkt" align='center' style='padding-bottom:0px;'>MY TICKET (#<?php echo $tid2; ?>)</h4>
				<div id='check_err' align='center'>Click on a number in ticket to check.</div>
				...
				<div class="accordion accordion-secondary" id="accordion2Primary">
								<div class="card card-default">
									<div class="card-header">
										<h4 class="card-title m-0" align='center'>
											<a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion2Primary" href="#collapse2PrimaryOne"  onclick='show_cbuttons();'>
												<i class='fas fa-gift'></i> CLAIM PRIZE HERE
											<i class='fa fa-angle-down' style='text-align:right'></i>
											</a>
										</h4>
									</div>
									<div id="collapse2PrimaryOne" class="collapse hide">
              ...
              ?>			  
				...
				<hr>
	<!--			<div align='center'>Latest 10 Numbers</div> -->
								<div align='center'><span id='recent'></span></div>				
...
<div>
					  ...
					  <h4 align='center' style='color:#000000;'>SEND MESSAGE TO ALL</h4>
					  <div class="input-group mb-3" style='padding-top:10px;'>
						<input type="text" id='msg1' class="form-control" placeholder='Write Comment / Message..' data-plugin-maxlength maxlength="1000" REQUIRED>
							...
							<section class="card">
								...
								for($i=1;$i<10;$i++)
								{
								  ...
								  for($j=1;$j<=10;$j++)
								  {
								   ...
								   {
									 echo "<div id='ticked'><i class='fa fa-check'></i></div>";
								   }else
								   {									   
									 echo $no;
								   }
...
<!--										<span class="badge badge-primary font-weight-normal va-middle p-2 mr-2" id='cmt_cnt'><?php //echo $cnt; ?></span> -->
								...
								<div class="card-footer">
											<div class="input-group mb-3" style='padding-top:10px;'>
														<input type="text" id='msg2' class="form-control" placeholder='Write Comment / Message..' data-plugin-maxlength maxlength="100" REQUIRED>
														<span class="input-group-append">
...
{   
  ...
  {
	document.getElementById("check_err").innerHTML="<span style='color:blue;'>The number " + tkt_no + " checking in progress!</span>";
  }
}
   ...
   if (xmlhttp.readyState==4 && xmlhttp.status==200)
    {
	  ...
	  else
	  {
		...
		if(win_type == "J5") { 
  	    ...
  	    if(win_type == "C6") { 
	       document.getElementById("claim_err").innerHTML="<span style='color:green;'><b>CONGRATS! YOU WON 6 CORNERS PRIZE</b></span>"; 
     	   document.getElementById(type).disabled=true; 
		   document.getElementById(type).innerHTML="<i class='fas fa-gift'></i> <strike>6 CORNERS</strike>";
	       }
  	    if(win_type == "CS") { 
	       document.getElementById("claim_err").innerHTML="<span style='color:green;'><b>CONGRATS! YOU WON CENTRE STAR PRIZE</b></span>"; 
     	   document.getElementById(type).disabled=true; 
		   document.getElementById(type).innerHTML="<i class='fas fa-gift'></i> <strike>CENTRE STAR</strike>";
	       }
  	    if(win_type == "FH") { 
...
<script>
        ...
        source.onmessage = function(event) {
		   ...
		   for(i=0;i<res.length;i++)
			 {
          	   x=res[i];
               if(x == "J5A") {var y = document.getElementById("J5A"); y.disabled=true; y.style.textDecorationLine='line-through';}
               if(x == "L1A") {var y = document.getElementById("L1A"); y.disabled=true; y.style.textDecorationLine='line-through';}
               if(x == "L2A") {var y = document.getElementById("L2A"); y.disabled=true; y.style.textDecorationLine='line-through';}
               if(x == "L3A") {var y = document.getElementById("L3A"); y.disabled=true; y.style.textDecorationLine='line-through';}
               if(x == "CNA") {var y = document.getElementById("CNA"); y.disabled=true; y.style.textDecorationLine='line-through';}
               if(x == "C6A") {var y = document.getElementById("C6A"); y.disabled=true; y.style.textDecorationLine='line-through';}
               if(x == "CSA") {var y = document.getElementById("CSA"); y.disabled=true; y.style.textDecorationLine='line-through';}
               if(x == "FHA") {var y = document.getElementById("FHA"); y.disabled=true; y.style.textDecorationLine='line-through';}
...
function send_message(mno)
  {
	...
	xmlhttp.onreadystatechange=function()
		...
		xmlhttp.open("GET","send_message.php" + "?gid=" + gid  + "&msg=" + msg + "&mno=" + mno, true);
		xmlhttp.send();
	}	
	else
 	 {
...
<script>
  ...
  source.onmessage = function(event) {
		   ...
		   else if(event.data == "WN")
		    {
				display_cube("<span style='font-size:18px;'>WIN</span>");
				document.getElementById('gstatus').innerHTML = "NEW WINNER";
		    }
		   ...
		   else
		   {
				...
				var er="c" + event.data;			 
				document.getElementById(er).innerHTML = "<div id='ticked'><i class='fa fa-check'></i></div>";
				if(sound == 1 && event.data != current)
					{
  ...
  function display_cube(dmsg)
	 {
		 if(dmsg == "GR")
		 { 
           document.getElementById('f1').innerHTML = "<span style='font-size:20px;margin-top:0px;'>GET</span>";		   
           document.getElementById('f2').innerHTML = "<img src='img/sponsor.png'>";		   
           document.getElementById('f3').innerHTML = "<span style='font-size:20px;margin-top:0px;'>SET</span>";		   
           document.getElementById('f4').innerHTML = "<img src='img/sponsor.png'>";		   
           document.getElementById('f5').innerHTML = "<span style='font-size:20px;margin-top:0px;'>GO</span>";		   					 
		 }
		 else
		 {	 
...
<script>
 var source = new EventSource("join_alert.php");
  source.onmessage = function(event) {
	  if(event.data != "0")
	  {
		new PNotify({
			title: 'New Game Join!',
			text: event.data,
            addclass: 'green notification-primary',
			icon: 'fab fa-tweet',
			delay:1000
		   });
	  }
...
<script>
  ...
  var source = new EventSource("game_comments.php?gid=<?php echo $gid; ?>");
  source.onmessage = function(event) {
	  if(event.data != "0")
	  {  
		new PNotify({
			title: 'New Player Comment!',
			text: event.data,
            addclass: 'blue notification-primary',
			icon: 'fab fa-twitter',
			delay:2000
		   });
	  }
...
<script>
function show_cbuttons()
{
   setTimeout(function(){
        $('#collapse2PrimaryOne').collapse('hide');
   }, 5000);  
